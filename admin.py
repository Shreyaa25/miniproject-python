...
class Admin_Login_System:
    def __init__(self,root):
        ...
        title=tk.Label(login_frame,text="Admin Login System",font=("times new roman",25,"bold"),bg="white")
        title.place(x=0,y=30,relwidth=1)
        
        lbl_username=tk.Label(login_frame,text="Username:",font=("times new roman",15),bg="white",fg="#767171")
        lbl_username.place(x=50,y=100)
        self.username=tk.StringVar()
        
        txt_username=tk.Entry(login_frame,textvariable=self.username,font=("times new roman",15),bg="#ECECEC")
        ...
        btn_close=tk.Button(login_frame,command=self.close,text="X",font=("times new roman",15),bg="#00B0F0",activebackground="#00B0F0",fg="white",activeforeground="white",cursor="hand2")
        btn_close.place(x=315,y=7,width=25,height=25)
        ...
        btn_forget=tk.Button(login_frame,text="Forgot Password?",command=self.forget_password_window,font=("times new roman",13),bg="white",fg="#00759E",bd=0,activebackground="white",activeforeground="#00759E")
    ...
    def forget_password(self):
        if self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_new_pass.get()=="":
            ...
            try:
                con=pymysql.connect(host="localhost",user="root",password="767686",database="wms")
                cur=con.cursor()
                cur.execute("select * from regfields where username=%s and question=%s and answer=%s",(self.username.get(),self.cmb_quest.get(),self.txt_answer.get()))
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Please Select Correct Security Question / Enter Answer",parent=self.root2)
                else:
                    cur.execute("update regfields set password=%s where username=%s",(self.txt_new_pass.get(),self.username.get()))
                    con.commit()
                    con.close()
    ...
    def forget_password_window(self):
        if self.username.get()=="":
            ...
            try:
                con=pymysql.connect(host="localhost",user="root",password="767686",database="wms")
                cur=con.cursor()
                cur.execute("select * from regfields where username=%s",self.username.get())
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Please enter valid username to reset your password",parent=self.root)
                else:
    ...
    def login(self):
        if self.username.get()=="" or self.password.get()=="":
            ...
            try:
                con=pymysql.connect(host="localhost",user="root",password="767686",database="wms")
                cur=con.cursor()
                cur.execute("select * from regfields where username=%s and password=%s",(self.username.get(),self.password.get()))
                row=cur.fetchone()
                if row==None:
