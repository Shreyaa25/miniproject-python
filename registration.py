...
class Register:
    def __init__(self,root):
        ...
        username=tk.Label(frame1,text="Username",font=("times new roman",15,"bold"),bg="white",fg="gray")
        username.place(x=430,y=170)
        
        self.txt_username=tk.Entry(frame1,font=("times new roman",15),bg="lightgray")
        self.txt_username.place(x=430,y=200,width=250)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_contact.get()=="" or self.txt_username.get()=="" or self.cmb_quest.get()=="Select" or self.txt_answer.get()=="" or self.txt_passwd.get()=="" or self.txt_cpasswd.get()=="":
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from regfields where username=%s",self.txt_username.get())
                row=cur.fetchone()
                #print(row)
                if row!=None:
                    messagebox.showerror("Error","User already exists with same username, register with another username.",parent=self.root)
                else:
                    cur.execute("insert into regfields (f_name,l_name,contact,username,question,answer,password) values(%s,%s,%s,%s,%s,%s,%s)",(self.txt_fname.get(),self.txt_lname.get(),self.txt_contact.get(),self.txt_username.get(),self.cmb_quest.get(),self.txt_answer.get(),self.txt_passwd.get()))
                    con.commit()
                    con.close()
